...
class SwinTransformerBackbone(BackboneBase):
    def __init__(self, swin_cfg_dict):
        # instantiate SwinTransformer
        backbone = SwinTransformer(**swin_cfg_dict)
        # strides
        all_strides = [4, 8, 16, 32]
        strides = [all_strides[i] for i in backbone.out_indices]
        # num channels
        num_channels = [backbone.num_features[i] for i in backbone.out_indices]

        super().__init__(backbone, strides, num_channels)
        if backbone.dilation:
...
def build_swin_backbone(cfg):
    ...
    kw = model_param_dict[cfg.model.backbone.name]
    model = SwinTransformerBackbone(kw)
    return model